...
const Weather = () => {
    ...
    return(
        <div className="fixed top-0 left-0 w-full bg-componentBg z-50 border-b-2 border-slate-500">
                ...
                <div className="flex flex-row bg-componentBg rounded justify-between w-full">
                    <div className="flex flex-row bg-componentBg items-center">
                        ...
                        <img src = {`https://openweathermap.org/img/wn/${data?.weather?.[0]?.icon}@2x.png`} alt="icon"></img>
                            ...
                            <div className="ml-2 text-gray-500 font-bold">{data?.weather?.[0]?.description}</div>
                        </div>
                        <div className="font-bold text-xl ml-8 text-textColor">{data?.name}
                        <div className="text-sm text-gray-500">{currentDay}   {currentTime} </div>
                        </div>
                    </div>
                    <div className="flex flex-row bg-componentBg rounded p-3">
                        <div className="font-medium text-3xl text-center text-textColor">{data?.main?.temp} &#8451;
                            <div className="flex flex-row items-center">
                                <div className="p-1">
                                    <span className="text-sm text-textColor">max: </span>
                                    <span className="text-sm text-gray-500 font-bold">{data?.main?.temp_max}</span>
                                </div>
                                <div className="p-1">
                                    <span className="text-sm text-textColor">min: </span>
                                    <span className="text-sm text-gray-500 font-bold">{data?.main?.temp_min}</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex flex-row justify-between">
                            <div className="flex flex-col items-center p-3">
                                ...
                                <div className="text-sm text-gray-500 font-bold">{data?.wind?.speed}</div>
                            </div>
                            <div className="flex flex-col items-center p-3">
                                <div className="font-medium text-sm text-textColor">Humidity</div>
                                <div className="text-sm text-gray-500 font-bold">{data?.main?.humidity}%</div>
                            </div>
                            <div className="flex flex-col items-center p-3">
                                <div className="font-medium text-sm text-textColor">Visibility</div>
                                <div className="text-sm text-gray-500 font-bold">{data?.visibility}m</div>
                            </div>
                        </div>
                    </div>
                </div>
